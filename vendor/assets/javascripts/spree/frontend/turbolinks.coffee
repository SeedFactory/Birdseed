...
fetch = (url, options = {}) ->
  ...
  if transitionCacheEnabled and !options.change and cachedPage = transitionCacheFor(url.absolute) and (not options.method or options.method is 'GET')
...
fetchReplacement = (url, options) ->
  ...
  xhr = $.ajax
    ...
    url: url.formatForXHR(cache: options.cacheRequest)
    xhrFields:
      onprogress: (event) ->
...
# The Scroll class aligns pages based on [data-scroll-target]
class Scroll
