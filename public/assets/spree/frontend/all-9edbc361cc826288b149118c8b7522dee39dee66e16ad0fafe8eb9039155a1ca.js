...
// Do this so the 300ms delay on iOS goes away.
...
(function() {
  ...
  fetch = function(url, options) {
    ...
    if (transitionCacheEnabled && !options.change && (cachedPage = transitionCacheFor(url.absolute) && (!options.method || options.method === 'GET'))) {
  ...
  fetchReplacement = function(url, options) {
    ...
    return xhr = $.ajax({
      ...
      complete: function(xhr) {
        var contentType, doc, loadedNodes;
  ...
  Submit = (function() {
    Submit.installHandlerLast = function(event) {
      if (!event.defaultPrevented) {
        document.removeEventListener('submit', Submit.handle, false);
        return document.addEventListener('submit', Submit.handle, false);
      }
    };

    Submit.handle = function(event) {
      return new Submit(event);
    };

    function Submit(event1) {
      var action, data, form, method, target;
      this.event = event1;
      this.event.preventDefault();
      Scroll.handle(this.event.target);
  ...
  Click = (function() {
    ...
    function Click(event1) {
      ...
      if ($(this.event.target).is(':submit')) {
        return new Submit(event);
      }
      this._extractLink();
